             ...
             tabPanel("VCF",
                      sidebarLayout(
                        ...
                        mainPanel(
                          ...
                          div(id = "inputStatus", style = "color: red; font-weight: bold;", HTML("It may take a while <span class='loading-dots'><span>•</span><span>•</span><span>•</span></span>")),
                          tags$hr(),
             ...
             tabPanel("data.frame/genlight",
                      sidebarLayout(
                        ...
                        mainPanel(
                          ...
                          div(id = "input2Status", style = "color: red; font-weight: bold;", HTML("It may take a while <span class='loading-dots'><span>•</span><span>•</span><span>•</span></span>")),
                          tags$hr(),
...
Page_1_Data_Input_Server = function(input, output, session) {
    ...
    if (guide_input() == "Now waiting for input VCF file... (Please click the 'Input VCF file' button)") {
      ...
      tryCatch({
        withProgress(message = 'Processing data...', value = 0.05, {
          ...
          # --- Check for VCFtools-like genotype fields ---
          if (!isTRUE(input$FVCFtools)) {
            check_cols = names(vcf)[10:min(12, ncol(vcf))]
            sample_rows = vcf[1:min(5, .N), ..check_cols]
            if (any(grepl(":", unlist(sample_rows)))) {
              showNotification(
                "Detected colon-separated genotype fields. If this file was generated by VCFtools, please check the 'VCF File from VCFtools' option.",
                type = "error",
                duration = 15
              )
            }
  ...
  observeEvent(input$vcf2df, {
    ...
    tryCatch({
      ...
      pre_results[[4]] = paste0("Number of samples: ", nrow(VCFdf()))
      pre_results[[5]] = paste0("Number of SNPs: ", ncol(VCFdf()))
      pre_results(pre_results)
  ...
  output$fileInfo = renderText({
    ...
    if (input1() == "VCF Data") {
      paste0("Type: VCF", "\n",
             "Number of samples: ", ncol(vcfData())-9, "\n",
             "Number of SNPs: ", nrow(vcfData()), "\n",
  ...
  output$fileInfo2 = renderText({
    ...
    if (input2() == "VCF Data in data.frame") {
      paste0("Type: data.frame", "\n",
             "Number of samples: ", nrow(VCFdf()), "\n",
             "Number of SNPs: ", ncol(VCFdf()), "\n",
             "File name: ", "data.frame_", nrow(VCFdf()), "_", ncol(VCFdf()), "SNPs", "\n",
  ...
  output$Ddf = downloadHandler(
    filename = function() {
      paste("data.frame_", nrow(VCFdf()), "_", ncol(VCFdf()), "SNPs.rds", sep = "")
    },
    content = function(file) {
      shinyjs::show("inputStatus")
      tryCatch({
  ...
  output$DsnpInfo = downloadHandler(
    filename = function() {
      paste("Site_Info_", nrow(VCFdf()), "_", ncol(VCFdf()), "SNPs.rds", sep = "")
    },
    content = function(file) {
      shinyjs::show("inputStatus")
      tryCatch({
